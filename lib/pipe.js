...
class Pipe extends Observation {
    ...
    return descriptorFields.reduce((descriptor, field) => {
      ...
      if (descriptor[field] && typeof descriptor[field] === 'function') {
        ...
        if (Pipe._isInstance(item)) {
          ...
          value = new Pipe(item)
          this._handleSubPipe(item['__pipe__'] || value, key)
        }
        return {
          ...descriptor,
          value
  ...
  /**
   * Handle origin store for predictable store
  ...
  create (initial = {}) {
    ...
    Reflect
      .ownKeys(this._model)
      .filter((key) => (
        typeof Reflect.getOwnPropertyDescriptor(this._model, key).value !== 'undefined' &&
        !['__parentPipe__', 'length', '__pipe__'].includes(key)
      ))
      .forEach((key) => {
        this._assign(this._model[key], key, initialValues)
      })
