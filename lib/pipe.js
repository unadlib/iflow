...
class Pipe extends Observation {
  ...
  constructor (originalModel) {
    ...
    this._model = new Proxy(originalModel, {
      set: this._setterProxy.bind(this),

      deleteProperty: function (target, property) {
        console.log('before Deleted %s', property)
        const result = Reflect.deleteProperty(target, property)
        console.log('after Deleted %s', property)
        return result
      },
  ...
  create (initial = {}) {
    ...
    if (
      ...
      toString.call(initial) === '[object Array]' &&
      initial.length > 0
    ) {
      initial.forEach((item, key) => {
        this._assign(item, key, initial)
      })
    }
