...
class Pipe extends Observation {
  ...
  _handleParentNode (item) {
    const isSettable = !isUnproxy(item._model) && !item._model.__parentPipe__
  ...
  _handleTypeFunc (fn, [beforeHook, afterHook], [item, model] = []) {
    if (toString.call(fn) === '[object AsyncFunction]' || fn.name === 'bound __async__') {
      return (async () => {
        let beforeHookResult = await (async () => beforeHook())()
    ...
    let beforeHookResult = beforeHook()
  ...
  _assign (item, key, initial) {
    ...
    } else if (Pipe._isInstance(item) && !Object.getOwnPropertyDescriptor(this._model, key).get) {
      this._model[key] = this._model[key].create(initial[key])
    } else if (Pipe._isInstantiable(item) && !Object.getOwnPropertyDescriptor(this._model, key).get) {
   ...
   */
  parent (number = 1) {
    return Array(number).fill('').reduce((prev) => {
  ...
  getState () {
    return Object
      ...
      .reduce((state, [key, value]) => {
        ...
        if (typeof value === 'object' && value.__pipe__) {
   ...
   */
  update () {
    this._applyHookAfterReflector(null, null, {mode: 'batch'})
  }
  ...
  create (initial = {}) {
    ...
    // root pipe
    const isSettable = !isUnproxy(this._model) && !this._model.__pipe__
