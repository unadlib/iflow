...
class Pipe extends Observation {
   ...
   * @private
   */
  _rebindModelAction (model, item, key, descriptor) {
    return (...args) => {
      const fn = item.bind(model, ...args, model)
      return this._handleTypeFunc(fn, [
        this.beforeActionHook.bind(this, key, model),
  ...
  _applyHookBeforeReflector (value, key, params) {
    ...
    if (typeof value === 'function' && params.mode === 'set') {
      const descriptor = Reflect.getOwnPropertyDescriptor(this._model, key)
      assignment = this._rebindModelAction(this._model, value, key, descriptor)
    }
